...
async def petrify(user, guild, reason, unlock_time = -1, unlock_chance = -1, unlock_interval = -1):
	...
	statue_active_role = sql_helpers.get_setting(guild.id,'statue_role')
	statue_candidate_role = sql_helpers.get_setting(guild.id,'statue_candidate_role')
	statue_admin_role = sql_helpers.get_setting(guild.id,'statue_admin_role')
	status = await sql_helpers.get_status(user.id, guild.id)

	petrify_success = False

	if status[0] == sql_helpers.Status.unpetrified:
		if reason == Reason.by_admin:
			await sql_helpers.set_status(user.id, guild.id, sql_helpers.Status.petrified_by_admin)
			text = f"{user.display_name} is petrified by anothers' magic."
			petrify_success = True

		elif reason == Reason.by_self_toggle:
			await sql_helpers.set_status(user.id, guild.id, sql_helpers.Status.petrified_by_self_toggle)
			text = f"{user.display_name} petrifies themselves."
			petrify_success = True

		elif reason == Reason.by_self_time:
			await sql_helpers.set_status(user.id, guild.id, sql_helpers.Status.petrified_by_self_time, unlock_time=unlock_time)
			...
			petrify_success = True
	elif status[0] != sql_helpers.Status.petrified_by_admin and reason == Reason.by_admin:
		await sql_helpers.set_status(user.id, guild.id, sql_helpers.Status.petrified_by_admin)
...
async def unpetrify(user, guild, reason):
	...
	if status[0] == sql_helpers.Status.unpetrified:
		...
		# nothing to unpetrify
			...
			pass
	elif status[0] == sql_helpers.Status.petrified_by_admin:
		if reason != Reason.by_admin:
			text = "You cannot unpetrify yourself if petrified by an admin."
		else:
			await sql_helpers.set_status(user.id, guild.id, sql_helpers.Status.unpetrified)
			text = f"{user.display_name} is unpetrified."
			unpetrify_success = True
	elif status[0] == sql_helpers.Status.petrified_by_self_toggle:
		if reason != Reason.by_self_toggle:
			...
			unpetrify_success = True
	elif status[0] == sql_helpers.Status.petrified_by_self_time:
		if reason != Reason.by_self_time:
			...
			unpetrify_success = True
	elif status[0] == sql_helpers.Status.petrified_by_self_chance:
